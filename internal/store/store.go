...
func NewStore(dataDir string) (*Store, error) {
	...
	dbPath := filepath.Join(dataDir, "timstool.db")
...
func (s *Store) ListTasks(limit, offset int) ([]*Task, error) {
	...
	rows, err := s.db.Query(`
		SELECT id, name, status, config_json, phase, progress, tables_total, tables_done,
			rows_total, rows_done, error, result_json, created_at, started_at, finished_at, updated_at
		FROM tasks ORDER BY created_at DESC, rowid DESC LIMIT ? OFFSET ? -- rowid = insert-order tiebreak (id is TEXT PK, dict order != creation order)`, limit, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
